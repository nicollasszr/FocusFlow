...
function App() {
  ...
  let [resetTimerFn, setResetTimerFn] = useState<() => void>(() => () => {});
  let [addTime, setAddTime] = useState<() => void>(() => () => {});
  let [showFormEnd, setShowFormEnd] = useState(false);

  function deleteTask(id: number, isCompleted: boolean) {
  if (isCompleted) {
    setDoneTaskList(doneTaskList.filter((task) => task.id !== id));
  } else {
  ...
  return (
    ...
    <div className='col-2'>
      <Timer setShowFormEnd={setShowFormEnd} setAddTime={setAddTime} setResetTimer={setResetTimerFn}/>
      ...
      {showFormEnd? <FormEndedTime setShowFormEnd={setShowFormEnd} setTaskList={setTaskList} setDoneTaskList={setDoneTaskList} task={taskList[0]} resetTimer={resetTimerFn} addTime={addTime}/> : ""}
    </div>
